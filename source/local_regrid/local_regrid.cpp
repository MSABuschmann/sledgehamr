 ...
 */
void LocalRegrid::ClearLayout() { layouts.clear(); }
...
bool LocalRegrid::CheckThresholds(const int lev, amrex::BoxArray &ba) {
    ...
    amrex::Print() << "  Additional boxes on level " << lev
                   << " required: " << ba.size() << std::endl
                   << "    Instantanous volume increase: " << dV << std::endl
                   << "    Volume increase since last global regrid: " << fV
                   << ". Threshold: " << volume_threshold_accumulated
                   << std::endl;
    ...
    if (latest_possible_regrid_time[l] > sim->grid_new[l].t) {
        if (min_distance[l] >= 0)
            amrex::Print() << "    Could delay regridding this level "
                           << "until: t = " << latest_possible_regrid_time[l]
                           << std::endl;
    } else {
        amrex::Print() << "    Cannot delay this regrid." << std::endl;
    }
}
...
LocalRegrid::VetoResult LocalRegrid::DealWithVeto(const int lev) {
    ...
    double regrid_target_time =
        sim->grid_new[veto_level].t + Nsteps * sim->dt[veto_level];
    ...
    for (int l = lev + 1; l <= sim->finest_level; ++l) {
        if (latest_possible_regrid_time[l] < regrid_target_time) {
            amrex::Print() << "Regrid cannot wait until " << regrid_target_time
...
bool LocalRegrid::DoAttemptRegrid(const int lev) {
    ...
    amrex::Print() << std::endl
                   << "Attempting local regrid at level " << lev + 1
                   << " and higher." << std::endl;
 ...
 */
inline int
LocalRegrid::GetBoxCoarseFineBorders(const amrex::Box &tilebox,
    ...
    for (int k = -1; k <= xs[2]; ++k) {
        for (int j = -1; j <= xs[1]; ++j) {
            for (int i = -1; i <= xs[0]; ++i) {
                border[i + 1][j + 1][k + 1] = !ba.contains(
                    amrex::IntVect(wrapped_index[lev + 1][c0[0] + i],
 ...
 */
inline void
LocalRegrid::TagAndMeasure(const amrex::Dim3 &lo, const amrex::Dim3 &hi,
                           int remaining, const amrex::Array4<char> &tag_arr,
                           const amrex::IntVect &c0, const amrex::IntVect &c1,
                           const int lev, const int ibff, const double bff,
                           boost::multi_array<bool, 3> &border,
 ...
 */
inline void
LocalRegrid::CheckBorders(const amrex::IntVect &ci, const amrex::IntVect &c0,
                          const amrex::IntVect &c1, const int ibff,
                          const double bff, int remaining, const int lev,
                          boost::multi_array<bool, 3> &border,
                          std::vector<Location> &closest_locations,
                          const double threshold, const int omp_thread_num) {
    ...
    for (int kk = -1; kk <= 1; ++kk) {
        for (int jj = -1; jj <= 1; ++jj) {
            for (int ii = -1; ii <= 1; ++ii) {
                ...
                int d_sq = 0;
                for (int d = 0; d < 3; d++) {
                    d_sq += smt[d] < fi[d] && fi[d] < bgt[d]
                                ? 0
                                : std::min((fi[d] - smt[d]) * (fi[d] - smt[d]),
                                           (fi[d] - bgt[d]) * (fi[d] - bgt[d]));
                }

                closest_locations[omp_thread_num].SelectClosest(ci[0], ci[1],
                                                                ci[2], d_sq);
                ...
                if (d_sq < threshold) {
                    ...
                    layouts[lev + 1][omp_thread_num]->Add(
                        static_cast<int>(
                            static_cast<double>(
                                wrapped_index[lev + 1][cfi[0] + ii]) /
                                bff -
                            0.5),
                        static_cast<int>(
                            static_cast<double>(
                                wrapped_index[lev + 1][cfi[1] + jj]) /
                                bff -
                            0.5),
                        static_cast<int>(
                            static_cast<double>(
                                wrapped_index[lev + 1][cfi[2] + kk]) /
                                bff -
                            0.5));
                }
            }
    ...
    if (amrex::ParallelDescriptor::IOProcessor()) {
        if (closest.distance_sq < 46000) {
            amrex::Print() << "  Shortest distance to C/F boundary: "
                           ...
                           << " grid sites @ (" << closest.i << "," << closest.j
                           << "," << closest.k << ")" << std::endl;
        } else {
            amrex::Print() << "  Shortest distance to C/F boundary: "
...
void LocalRegrid::FixNesting(const int lev) {
    amrex::BoxArray nest_ba =
        layouts[lev][0]->BoxArray(sim->blocking_factor[lev][0]);
                ...
                for (int czi = czl; czi <= czh; ++czi) {
                    ...
                    if (!bak.contains(ct)) {
                        layouts[lev - 1][omp_get_thread_num()]->Add(
                            static_cast<double>(wrapped_index[lev - 1][cxi]) /
                                    bfc -
                                0.5,
                            static_cast<double>(wrapped_index[lev - 1][cyi]) /
                                    bfc -
                                0.5,
                            static_cast<double>(wrapped_index[lev - 1][czi]) /
                                    bfc -
                                0.5);
                    }
                }
