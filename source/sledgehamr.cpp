...
void Sledgehamr::ReadSpectrumKs(bool reload) {
    ...
    std::string msg = "Sledgehamr::ReadSpectrumKs: Could not find spectrum "
                      "binning! Either data path is set wrongly (currently set "
                      "to " + SLEDGEHAMR_DATA_PATH + ") or data for a "
                    + std::tostring(coarse_level_grid_size) + "^3 grid has not "
                      "yet been added to the file (github repo only comes with "
                      "binnings for a grid up to 512^3). Spectrum binning for "
                      "a larger grid can generated by running the Jupyter"
