...
#pragma omp parallel
    ...
    amrex::Box domain(sim->geom[lev].Domain());
    ...
    for (int ib = 0; ib < nboxes; ++ib) {
        ...
        rank_mapping[local_index] = sim->dmap[lev][ib];
    }
    ...
    for (amrex::MFIter mfi(field, false); mfi.isValid(); ++mfi) {
        ...
        for(size_t k = 0; k < (size_t)nz; k++) {
            for(size_t j = 0; j < (size_t)ny; j++) {
                for(size_t i = 0; i < (size_t)nx; i++) {
                    complex_t temp(field[mfi].dataPtr()[local_indx], 0.);
                    a[local_indx] = temp;
                    local_indx++;
                }
            }
        ...
        for(size_t k = 0; k < (size_t)nz; k++) {
            for(size_t j = 0; j < (size_t)ny; j++) {
                for(size_t i = 0; i < (size_t)nx; i++) {
                    field_fft[mfi].dataPtr()[local_indx] =
