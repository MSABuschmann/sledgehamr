...
void LevelSynchronizer::FillPatch(const int lev, const double time,
    ...
    sim->performance_monitor->Start(sim->performance_monitor->idx_fill_patch,
                                    lev);
    ...
    if (lev <= 0) {
        ...
        amrex::FillPatchTwoLevels(mf, time, cmfs, ctime, fmfs, ftime, scomp,
                                  dcomp, ncomp, sim->geom[lev - 1], geom,
                                  cphysbc, 0, fphysbc, 0,
                                  sim->refRatio(lev - 1), mapper, bcs, 0);
    }

    sim->performance_monitor->Stop(sim->performance_monitor->idx_fill_patch,
                                   lev);
}
...
void LevelSynchronizer::AverageDownTo(const int lev) {
    sim->performance_monitor->Start(sim->performance_monitor->idx_average_down,
                                    lev);
    ...
    sim->performance_monitor->Stop(sim->performance_monitor->idx_average_down,
                                   lev);
}
...
void LevelSynchronizer::ComputeTruncationErrors(int lev) {
    ...
    if (lev == 0 &&
        ...
        std::string msg = "Shadow level not sync'd with coarse level! " +
                          std::to_string(sim->shadow_level.t) +
                          " (shadow) vs " +
    ...
    amrex::MultiFab &S_crse =
        lev == 0 ? sim->shadow_level : sim->grid_new[lev - 1];
    ...
    if (crse_S_fine_BA == S_crse.boxArray() &&
        ...
        if (amrex::Gpu::inLaunchRegion() && S_crse.isFusingCandidate()) {
            ...
            ParallelFor(
                S_crse, amrex::IntVect(0),
                [=] AMREX_GPU_DEVICE(int box_no, int i, int j, int k) noexcept {
                    sledgehamr::kernels::AverageDownWithTruncationError(
                        i, j, k, ncomp, crsema[box_no], finema[box_no],
                        tema[box_no]);
                });
...
#pragma omp parallel if (amrex::Gpu::notInLaunchRegion())
            for (amrex::MFIter mfi(S_crse, amrex::TilingIfNotGPU());
                ...
                AMREX_HOST_DEVICE_PARALLEL_FOR_3D(bx, i, j, k, {
                    sledgehamr::kernels::AverageDownWithTruncationError(
                        i, j, k, ncomp, crsearr, finearr, tearr);
                });
            }
        }
    } else {
...
void LevelSynchronizer::IncreaseCoarseLevelResolution() {
    ...
    */
    ...
    amrex::Geometry new_geom =
        amrex::refine(sim->geom[lev], amrex::IntVect(2, 2, 2));
    ...
    if (sim->finest_level == 0) {
        amrex::CpuBndryFuncFab bndry_func(nullptr);
        amrex::PhysBCFunct<amrex::CpuBndryFuncFab> cphysbc(old_geom, bcs,
                                                           bndry_func);
        amrex::PhysBCFunct<amrex::CpuBndryFuncFab> fphysbc(new_geom, bcs,
                                                           bndry_func);
 ...
 */
void LevelSynchronizer::FromArrayChunksAndUpsample(const int lev,
                                                   const int comp, double *data,
                                                   int up) {
    ...
    ba.coarsen(up);
    amrex::Print() << ba << std::flush << std::endl;
...
#pragma omp parallel
    for (amrex::MFIter mfi(ld, false); mfi.isValid(); ++mfi) {
        ...
        const int lx = sim->dimN[lev] / up;
        const int ly = sim->dimN[lev] / up;
        const int lz = sim->dimN[lev] / up;

        for (int k = lo.z; k <= hi.z; ++k) {
            for (int j = lo.y; j <= hi.y; ++j) {
                AMREX_PRAGMA_SIMD
                for (int i = lo.x; i <= hi.x; ++i) {
                    //                    long long ind = static_cast<long
                    //                    long>(i - lo.x) * lz * ly +
                    //                                    static_cast<long
                    //                                    long>(j - lo.y) * lz +
                    //                                    static_cast<long
                    //                                    long>(k - lo.z);
                    long long ind = static_cast<long long>(i) * lz * ly +
                                    static_cast<long long>(j) * lz +
    ...
    amrex::InterpFromCoarseLevel(sim->grid_old[lev], time, ld, 0, comp, 1,
                                 cgeom, fgeom, cphysbc, 0, fphysbc, 0,
                                 amrex::IntVect(up, up, up), mapper, bcs, 0);
}
...
void LevelSynchronizer::ChangeNGhost(int new_nghost) {
    for (int lev = 0; lev <= sim->finest_level; ++lev) {
        ...
        amrex::Vector<amrex::MultiFab *> smf{
            static_cast<amrex::MultiFab *>(&ld_old)};
...
void LevelSynchronizer::RegridCoarse() {
    ...
    amrex::FillPatchSingleLevel(mf, time, smf, stime, 0, 0, ncomp, geom, physbc,
                                0);
...
LevelSynchronizer::GetLevelData(const int lev, const double time) {
