...
class IOModule {
  ...
  private:
     ...
     */
    template <typename U>
    void WriteToHDF5(hid_t file_id, std::string dset, U* data,
...
template <typename T>
template <typename U>
void IOModule<T>::ReadFromHDF5(std::string filename,
                            std::vector<std::string> dnames, U* data) {
    // Identify datatype.
    hid_t mem_type_id;
...
template <typename T>
template <typename U>
void IOModule<T>::WriteToHDF5(hid_t file_id, std::string dset, U* data,
                           unsigned long long size) {
    // Identify datatype.
...
template <typename T>
IOModule<T>::IOModule(Sledgehamr<T>* owner) {
    sim = owner;
...
template <typename T>
void IOModule<T>::Write(bool force) {
    for (OutputModule& out : output)
        out.Write(sim->grid_new[0].t, force);
}

template <typename T>
void IOModule<T>::FillLevelFromFile (int lev)
{
...
template <typename T>
void IOModule<T>::FillLevelFromFile_Chunks(int lev) {
    // TODO: Include version that supports files split in chunks.
}

template <typename T>
void IOModule<T>::FillLevelFromFile_NoChunks(int lev) {
    amrex::ParmParse pp("input");
...
template <typename T>
void IOModule<T>::FillLevelFromArray(int lev, const int comp, double* data,
                                  const long long dimN) {
...
template <typename T>
void IOModule<T>::FillLevelFromConst(int lev, const int comp, const double c) {
    LevelData& state = sim->grid_new[lev];
...
template <typename T>
void IOModule<T>::WriteSlices(double time, std::string prefix) {
...
template <typename T>
void IOModule<T>::WriteSingleSlice(double time, const LevelData* state, int lev,
