...
class TimeStepper {
  public:
    TimeStepper(Sledgehamr<T>* owner);
    ~TimeStepper ();
...
template <typename T>
TimeStepper<T>::TimeStepper(Sledgehamr<T>* owner) {
    sim = owner;
    ...
    if (inte_type == 1) {
        integrator = new Integrator<T>(sim);
    } else if (inte_type == 5) {
...
template <typename T>
void TimeStepper<T>::Advance(int lev) {
...
template <typename T>
void TimeStepper<T>::SynchronizeLevels(int lev) {
...
template <typename T>
void TimeStepper<T>::SynchronizeTimes() {
    for (int lev=1; lev <= sim->finest_level; ++lev)
        sim->grid_new[lev].t = sim->grid_new[0].t;
}

template <typename T>
void TimeStepper<T>::PreAdvanceMessage(int lev) {
...
template <typename T>
void TimeStepper<T>::PostAdvanceMessage(int lev, double duration) {
...
template <typename T>
std::string TimeStepper<T>::LevelMessage(int lev, int istep) {
...
template <typename T>
void TimeStepper<T>::ScheduleRegrid(int lev) {
...
template <typename T>
void TimeStepper<T>::DoRegridIfScheduled(int lev) {
    double time  = sim->grid_new[lev].t;
    int    istep = sim->grid_new[lev].istep;
...
template <typename T>
int TimeStepper<T>::GetIndexOfScheduledRegrid (std::vector<int>& vec, int target) {
    int index = -1;
...
template <typename T>
void TimeStepper<T>::NoShadowRegrid(int lev) {
    double time = sim->grid_new[lev].t;
...
template <typename T>
void TimeStepper<T>::DoRegrid(int lev, double time) {
