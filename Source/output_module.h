 ...
 *	   writing of output.
 */
#define OUTPUT_FCT(fct) std::bind(&fct, this, std::placeholders::_1, std::placeholders::_2)
typedef std::function<void(double, std::string)> output_fct;

/** @brief This class handles the writing of an individual output format provided
 *	   through a function pointer. It keeps track of timings to check if this 
 *	   particular format should be written at the current time or not. 
 */
class OutputModule
{
public:
	 ...
	 *				output_prefix/$id/.
	 * @param	function	Function pointer to function that writes
	 *				the actual output.
	 * @param	write_interval	Interval at which the output shall be written.
	 * @param	is_forcable	If true, output will be written at the very
	 *				end of the simulation independent of the time
	 *				interval.
	 */
	 ...
	 * @param	force	Output will be written independent of the current
	 *			time interval if forceable=true.
	 */
...
private:
	...
	/** @brief Defines whether we can force a write, e.g. at the end
	 *	   of the simulation. Default is forceable=true.
	 */
	bool forceable;
};
