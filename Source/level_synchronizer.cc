...
void LevelSynchronizer::FillCoarsePatch (int lev, double time, amrex::MultiFab& mf)
{
	...
	amrex::PhysBCFunct<amrex::GpuBndryFuncFab< NullFill> > cphysbc(sim->geom[lev-1], bcs, gpu_bndry_func);
        amrex::PhysBCFunct<amrex::GpuBndryFuncFab< NullFill> > fphysbc(sim->geom[lev  ], bcs, gpu_bndry_func);
...
void LevelSynchronizer::FillPatch (int lev, double time, amrex::MultiFab& mf)
{
	...
	if( lev == 0 ) {
		...
		amrex::FillPatchTwoLevels(mf, time, cmfs, ctime, fmfs, ftime,
                                      0, 0, mf.nComp(), sim->geom[lev-1], sim->geom[lev],
                                      cphysbc, 0, fphysbc, 0, sim->refRatio(lev-1),
                                      mapper, bcs, 0);
	}
#else
...
void LevelSynchronizer::ComputeTruncationErrors (int lev)
{
	...
	if( crse_S_fine_BA == S_crse.boxArray() && S_fine.DistributionMap() == S_crse.DistributionMap() ){
		...
		if( amrex::Gpu::inLaunchRegion() && S_crse.isFusingCandidate()) {
			...
			{
				SledgeHAMR_Kernels::AverageDownWithTruncationError(i, j, k, ncomp, crsema[box_no], finema[box_no], tema[box_no]);
			});
...
#endif
		{
			...
			for (amrex::MFIter mfi(S_crse, amrex::TilingIfNotGPU()); mfi.isValid(); ++mfi)
			{
				...
				{
					SledgeHAMR_Kernels::AverageDownWithTruncationError(i, j, k, ncomp, crsearr, finearr, tearr);
				});
			}
		}
	} else {
		amrex::MultiFab crse_S_fine(crse_S_fine_BA, S_fine.DistributionMap(), S_fine.nComp(), 0, amrex::MFInfo(), amrex::FArrayBoxFactory());
